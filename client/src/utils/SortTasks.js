...
export default function SortTasks(tempArr) {
  ...
  while (tempArr.length > 0) {
    ...
    if (new Date(tempArr[0].startTime) == null || new Date(tempArr[0].startTime) < nextAvailableStart || tempArr[0].isPinnedStartTime == false) {
      ...
      axios
        .patch(
          `http://localhost:8000/api/tasks/${tempArr[0]._id}`,
          tempArr[0]
        )
    ...
    if(taskEndTime(tempArr[0]) < new Date()){
      ...
      tempArr[0].actualTotalDuration = tempArr[0].durationOfTask+tempArr[0].durationOfBreak;
    } else {
